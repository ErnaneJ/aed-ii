...
class Inventory:
    ...
    def find_laptops_with_features(self, features):
        filtered_features = {key: value for key, value in features.items() if key in self.header}

        matching_laptops = []

        for row in self.rows:
            is_match = all(str(value) in str(row[self.header.index(key)]) for key, value in filtered_features.items())
            if is_match:
                matching_laptops.append(row)

        print(f"\n.:: {len(matching_laptops)} laptops were found with the reported configurations.")
        print(f"\n{tabulate(matching_laptops, headers=self.header)}\n")
...
if __name__ == "__main__":
    ...
    inventory.find_laptops_with_features(desired_features)